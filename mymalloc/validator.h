...
static int add_range(const malloc_impl_t* impl, range_t** ranges, char* lo,
  ...
  if ((uint64_t) my_heap_hi() < (uint64_t) hi || (uint64_t) my_heap_lo() > (uint64_t) lo) {
    printf("Error: %p not in the heap range\n", lo);
    return 0;
  }
  ...
  while (q != NULL) {
    q_hi = q->hi;
    q_lo = q->lo;
    if ((lo >= q_lo && lo <= q_hi) ||  (hi >= q_lo && hi <= q_hi)){
       printf("Error: %p intersects with another range\n", lo);
       return 0;
    }
    q = q->next;
  }
    ...
    switch (trace->ops[i].type) {
      case ALLOC:  // malloc
        ...
        if (add_range(impl, &ranges, p, size, tracenum, i) == 0) {
          printf("when trying to add trace %d with size %d \n", i, size);
          return 0;
        }
      ...
      case REALLOC:  // realloc
        ...
        for (int i = 0; i < oldsize; ++i) {
           if (*(newp + i) != i%128) {
              printf("%d %d\n", (int)*(newp + i), i%128);
              printf("Error: %p not well realloced (index %d)\n", newp + i, i);
              return 0;
           }
        }
